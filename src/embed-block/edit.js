...
/**
 * Edit component for the chat embed block
 */
export default function Edit( { attributes, setAttributes } ) {
	...
	const blockProps = useBlockProps( {
		className: 'gt-librarian-chat-embed-block',
	} );
	...
	return (
		<>
			<InspectorControls>
				...
				>
					<ToggleControl
						label={ __( 'Hide Input Area', 'gt-librarian' ) }
						checked={ hideInputArea }
						onChange={ ( value ) =>
							setAttributes( { hideInputArea: value } )
						}
						help={ __(
							'Hide the message input area (read-only mode)',
							'gt-librarian'
						) }
					/>
					<ToggleControl
				...
				{ bot && ! loading && (
					<div className="gt-librarian-preview">
						<div className="gt-librarian-preview-header">
							<strong>
								{ __( 'GT Librarian Chat Embed', 'gt-librarian' ) }
							</strong>
						</div>
						<div className="gt-librarian-preview-body">
							...
							<p className="gt-librarian-preview-note">
								{ __(
									'The chat widget will appear here on the frontend.',
									'gt-librarian'
								) }
							</p>
						</div>
